...
class RailPathSegment extends Reference:
    ...
    func get_increased_position(position: Vector2, increase: Vector2) -> Array:
        # Requires a position (in pixels, relative to the top left of the object)
        # and the amount to increase (also in pixels)
        # Returns two vectors: (new_position, remaining_increase)
        # If remaining_increase is not 0, then you'll need to find out in which tile you are now,
        # what object is there, recalculate the offset position and check again for the increased position
        return [position, Vector2(0, 0)]  # This derails the train. Overridden in subclasses.    


class _ShortVerticalSegment extends RailPathSegment:
    
    func get_increased_position(position: Vector2, increase: Vector2) -> Array:
        var new_y = position.y + increase.y
        if new_y < 0:
            # Need to continue on previous tile
            if new_y > -_DELTA:
                # Stop juuuuust before this tile starts
                return [Vector2(_TILE_LENGTH / 2.0, -_DELTA), Vector2(0, 0)]
            return [Vector2(-_TILE_LENGTH / 2.0, -_DELTA), Vector2(0, new_y + _DELTA)]
        if new_y >= _TILE_LENGTH:
            # Need to continue on next tile
            return [Vector2(_TILE_LENGTH / 2.0, _TILE_LENGTH), Vector2(0, new_y - _TILE_LENGTH)]
        # Stop within bounds
        return [Vector2(_TILE_LENGTH / 2.0, new_y), Vector2(0, 0)]
...
class _RotatedSegment extends RailPathSegment:
    ...
    func get_increased_position(position: Vector2, increase: Vector2):
        var rotated_position = self._rotate(position)
        var result = self._original_segment.get_increased_position(rotated_position, increase)
...
static func rotated_rail(rail: RailPathSegment, rotation: int) -> RailPathSegment:
